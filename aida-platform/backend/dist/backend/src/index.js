...
import { createWebhookHandler } from './evolution-api/webhook-handler';
import { createMessageFormatter } from './evolution-api/message-formatter';
import { createVectorSearchEngine } from './rag/vector-search';
import { createEmbeddingService } from './rag/embedding-service';
import { createHybridQueryEngine } from './rag/hybrid-query';
import { createLangChainProcessor } from './ai/langchain-setup';
import { createResponseGenerator } from './ai/response-generator';
// API routes
...
app.use('*', async (c, next) => {
    try {
        ...
        return c.json({ error: 'Platform initialization failed', details: error instanceof Error ? error.message : String(error) }, 500);
    }
});
// Mount API routes
