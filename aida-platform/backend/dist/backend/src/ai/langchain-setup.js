...
class AIDARetriever extends BaseRetriever {
    hybridQueryEngine;
    businessId;
    constructor(hybridQueryEngine, businessId) {
        super();
...
export class LangChainProcessor {
     ...
     */
    async processMessage(message, context, options = this.getDefaultOptions()) {
        ...
        try {
            ...
            const chain = await this.createProcessingChain(prompt, memory, context, options);
            // Execute the chain
     ...
     */
    async createProcessingChain(prompt, memory, context, options) {
        if (options.includeRAG && this.config.ragEnabled && this.config.hybridQueryEngine) {
            // RAG-enabled chain
            const retriever = new AIDARetriever(this.config.hybridQueryEngine, context.businessId);
            return RunnableSequence.from([
                {
                    input: (input) => input.input,
                    context: retriever,
                    chat_history: (input) => memory.chatHistory || []
                },
                prompt,
                this.llm,
...
Respond as ${assistant.name} would, using the provided context and conversation history.`;
     ...
     */
    getOrCreateMemory(conversationId) {
     ...
     */
    async loadConversationHistory(conversationId, memory) {
        try {
            // Get recent messages from database
            const messages = await this.config.supabase.query('messages', 'content, sender_type, timestamp', {
            ...
            for (const message of messages.slice(-this.config.memoryK * 2)) { // Load more than K to account for pairs
                if (message.sender_type === 'customer') {
                    await memory.saveContext({ input: message.content }, { output: '' } // Will be filled by next assistant message
                    );
                }
     ...
     */
    checkEscalationNeeded(message, escalationKeywords) {
        const messageLower = message.toLowerCase();
        // Check for explicit escalation keywords
        const keywordMatch = escalationKeywords.some(keyword => messageLower.includes(keyword.toLowerCase()));
        ...
        const emotionalMatch = emotionalIndicators.some(indicator => messageLower.includes(indicator));
    ...
    calculateResponseConfidence(response, context) {
        ...
        if (response.sources && response.sources.length > 0) {
            const avgSourceScore = response.sources.reduce((sum, source) => sum + (source.score || 0), 0) / response.sources.length;
     ...
     */
    createEscalationResponse(message, context, startTime) {
     ...
     */
    createFallbackResponse(message, context, startTime) {
        return {
 ...
 */
export function createLangChainProcessor(config) {
    return new LangChainProcessor(config);
}
/**
 * Default LangChain configuration
 */
export function getDefaultLangChainConfig(supabase, hybridQueryEngine) {
    return {
        provider: 'openai',
        model: 'gpt-4o-mini',
