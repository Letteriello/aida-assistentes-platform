...
import { ConversationUpdateSchema, ExportRequestSchema, MessageCreateSchema, PaginationSchema } from '@shared/schemas';
const app = new Hono();
 ...
 */
function calculateConversationAnalytics(conversations) {
    const totalMessages = conversations.reduce((sum, conv) => sum + (conv.messages?.length || 0), 0);
    const totalCustomerMessages = conversations.reduce((sum, conv) => sum + (conv.messages?.filter((m) => m.sender_type === 'customer')?.length || 0), 0);
    const totalAssistantMessages = conversations.reduce((sum, conv) => sum + (conv.messages?.filter((m) => m.sender_type === 'assistant')?.length || 0), 0);
    return {
        totals: {
        ...
        averages: {
            ...
            responseTime: conversations.reduce((sum, conv) => sum + calculateAverageResponseTime(conv.messages || []), 0) / Math.max(conversations.length, 1)
        },
        trends: {
 ...
 */
function formatConversationsAsCSV(conversations) {
    const headers = [
        'ID',
        'Assistant',
        'Customer Name',
