...
export class ContextManager {
    ...
    contextCache;
    stats;
    constructor(config, supabase, env, embeddingService, vectorSearch) {
        this.config = config;
    ...
    async getEnhancedContext(request) {
        ...
        try {
            ...
            const contextWindow = await this.getContextWindow(request.conversationId, request.businessId, request.userId);
     ...
     */
    async addConversationTurn(conversationId, businessId, userQuery, systemResponse, metadata = {}, userId) {
        try {
     ...
     */
    async getContextWindow(conversationId, businessId, userId) {
        // Check cache first
     ...
     */
    async analyzeQuery(query, contextWindow) {
        try {
     ...
     */
    async retrieveContext(request, contextWindow) {
        const startTime = Date.now();
        ...
        if (contextTypes.includes('conversation')) {
            result.conversationHistory = await this.getRelevantConversationHistory(request.query, contextWindow, request.maxResults || 5);
            result.relevantContext.push(...result.conversationHistory.map(turn => `User: ${turn.userQuery}\nAssistant: ${turn.systemResponse}`));
        }
        // Retrieve document context
        if (contextTypes.includes('documents')) {
            result.relatedDocuments = await this.getRelevantDocuments(request.query, request.businessId, contextWindow, request.maxResults || 10);
            result.relevantContext.push(...result.relatedDocuments.map(doc => doc.content));
        }
        // Retrieve entity context
        if (contextTypes.includes('entities')) {
            result.entities = await this.getRelevantEntities(request.query, contextWindow);
            result.relevantContext.push(...Object.entries(result.entities).map(([key, value]) => `${key}: ${JSON.stringify(value)}`));
        }
     ...
     */
    async getRelevantConversationHistory(query, contextWindow, maxResults) {
     ...
     */
    async getRelevantDocuments(query, businessId, contextWindow, maxResults) {
        try {
     ...
     */
    async getRelevantEntities(query, contextWindow) {
     ...
     * PATTERN: Context state management
     */
    async updateContextWindow(contextWindow) {
        try {
     ...
     */
    async pruneContextIfNeeded(contextWindow) {
        try {
            ...
            const importantTurns = contextWindow.turns.filter(turn => turn.metadata.confidence > 0.8 ||
     ...
     */
    async persistContextWindow(contextWindow) {
        try {
     ...
     */
    async loadContextWindow(conversationId, businessId) {
        try {
     ...
     * PATTERN: Context-aware query analysis
     */
    analyzeQueryReferences(query, contextEntities, contextTopics) {
        const queryLower = query.toLowerCase();
        const entityReferences = contextEntities.filter(entity => queryLower.includes(entity.toLowerCase()));
        const topicReferences = contextTopics.filter(topic => queryLower.includes(topic.toLowerCase()));
        return {
            entityReferences,
            topicReferences,
     ...
     */
    async generateQueryExpansions(query, contextWindow) {
        const expansions = [];
        try {
     ...
     */
    createContextualFilters(queryAnalysis, contextWindow) {
        const filters = {};
     ...
     */
    recommendSearchStrategy(query, contextWindow) {
        ...
        const hasContextReferences = contextWindow.topics.some(topic => query.toLowerCase().includes(topic.toLowerCase()));
     ...
     */
    optimizeQueryWithContext(query, contextWindow, expansions) {
     ...
     */
    calculateContextConfidence(result, query) {
        let confidence = 0;
     ...
     */
    enhanceQueryWithContext(query, contextWindow) {
     ...
     */
    updateContextStats(processingTime, contextLength) {
     ...
     */
    async extractContextFromTurn(userQuery, systemResponse) {
        const context = [];
     ...
     */
    async generateContextSummary(contextWindow) {
        if (contextWindow.turns.length === 0) {
            return '';
        }
     ...
     */
    async extractTopics(contextWindow) {
        const topics = [];
        // Extract topics from recent turns
     ...
     */
    async extractEntities(contextWindow) {
        const entities = {};
     ...
     */
    calculateTotalTokens(contextWindow) {
        let totalTokens = 0;
        for (const turn of contextWindow.turns) {
     ...
     */
    extractTermsFromRecentTurns(contextWindow, maxTurns) {
 ...
 */
export function createContextManager(config, supabase, env, embeddingService, vectorSearch) {
