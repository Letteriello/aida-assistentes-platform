...
describe('EvolutionApiClient', () => {
    ...
    describe('sendTextMessage', () => {
        it('should send text message successfully', async () => {
            ...
            const result = await client.sendTextMessage('test-instance', '5511999999999@s.whatsapp.net', 'Hello World');
            expect(result).toEqual(mockResponse);
            expect(mockFetch).toHaveBeenCalledWith('http://localhost:8080/message/sendText/test-instance', expect.objectContaining({
                method: 'POST',
        ...
        it('should validate WhatsApp JID format', async () => {
            await expect(client.sendTextMessage('test-instance', 'invalid-jid', 'Hello')).rejects.toThrow('Invalid WhatsApp JID format');
        });
        it('should validate message text', async () => {
            await expect(client.sendTextMessage('test-instance', '5511999999999@s.whatsapp.net', '')).rejects.toThrow('Message text cannot be empty');
            await expect(client.sendTextMessage('test-instance', '5511999999999@s.whatsapp.net', 'a'.repeat(4001))).rejects.toThrow('Message text exceeds maximum length');
        });
        it('should handle API errors gracefully', async () => {
            ...
            await expect(client.sendTextMessage('test-instance', '5511999999999@s.whatsapp.net', 'Hello')).rejects.toThrow('Invalid Evolution API key');
        });
        it('should handle rate limiting', async () => {
            ...
            await expect(client.sendTextMessage('test-instance', '5511999999999@s.whatsapp.net', 'Hello')).rejects.toThrow('Rate limit exceeded by Evolution API');
        });
    });
    describe('createInstance', () => {
        it('should create WhatsApp instance successfully', async () => {
            ...
            expect(result).toEqual(mockResponse);
            expect(mockFetch).toHaveBeenCalledWith('http://localhost:8080/instance/create', expect.objectContaining({
                method: 'POST',
    ...
    describe('setWebhook', () => {
        ...
        it('should validate webhook URL', async () => {
            await expect(client.setWebhook('test-instance', {
                webhook: 'invalid-url',
                webhookEvents: [],
                webhookBase64: false
