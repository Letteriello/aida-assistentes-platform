...
import { getDefaultVectorSearchConfig } from '../../../rag/vector-search';
// Mock TenantAwareSupabase
class MockTenantAwareSupabase {
    businessId;
    constructor(businessId = 'test-business-123') {
        this.businessId = businessId;
    }
    async vectorSearch(table, embedding, limit, threshold, filters) {
...
describe('VectorSearchEngine', () => {
    ...
    describe('search', () => {
        ...
        it('should validate embedding dimensions', async () => {
            ...
            await expect(searchEngine.search(query, invalidEmbedding)).rejects.toThrow('Invalid embedding: expected 384 dimensions');
        });
    ...
    describe('searchConversations', () => {
        it('should search conversations with proper filters', async () => {
            const testEmbedding = new Array(384).fill(0.1);
            const results = await searchEngine.searchConversations(testEmbedding, 0.7, 5);
            expect(results).toBeDefined();
            expect(Array.isArray(results)).toBe(true);
        });
        it('should filter by conversation ID when provided', async () => {
            const testEmbedding = new Array(384).fill(0.1);
            const results = await searchEngine.searchConversations(testEmbedding, 0.7, 5, 'specific-conversation-id');
            expect(results).toBeDefined();
        });
        it('should filter by time range when provided', async () => {
            ...
            const results = await searchEngine.searchConversations(testEmbedding, 0.7, 5, undefined, timeRange);
            expect(results).toBeDefined();
        });
    });
    describe('searchMessages', () => {
        it('should search messages successfully', async () => {
            const testEmbedding = new Array(384).fill(0.1);
            const results = await searchEngine.searchMessages(testEmbedding, 0.7, 5);
            expect(results).toBeDefined();
            expect(Array.isArray(results)).toBe(true);
        });
    });
    describe('searchKnowledge', () => {
        it('should search knowledge base successfully', async () => {
            const testEmbedding = new Array(384).fill(0.1);
            const results = await searchEngine.searchKnowledge(testEmbedding, 0.7, 5);
            expect(results).toBeDefined();
            expect(Array.isArray(results)).toBe(true);
        });
        it('should filter by entity types when provided', async () => {
            const testEmbedding = new Array(384).fill(0.1);
            const results = await searchEngine.searchKnowledge(testEmbedding, 0.7, 5, ['product', 'service']);
            expect(results).toBeDefined();
        });
    });
    describe('findSimilarConversations', () => {
        it('should find similar conversations', async () => {
            ...
            const results = await searchEngine.findSimilarConversations('test-conversation-id', 3);
            expect(results).toBeDefined();
            expect(Array.isArray(results)).toBe(true);
        });
        it('should return empty array when no embeddings found', async () => {
            vi.spyOn(mockSupabase, 'query').mockResolvedValueOnce([]);
            const results = await searchEngine.findSimilarConversations('nonexistent-conversation-id', 3);
...
describe('Factory functions', () => {
    ...
    it('should provide default configuration', () => {
        const mockSupabase = new MockTenantAwareSupabase();
        const config = getDefaultVectorSearchConfig(mockSupabase);
