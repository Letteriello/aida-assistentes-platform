 ...
 */
async function generateApiKeys(businessId: string, env: Env): Promise<{ liveApiKey: string; testApiKey: string }> {
 ...
 */
async function revokeApiKeys(businessId: string, env: Env, apiKeys: string[]): Promise<number> {
  const supabase = new TenantAwareSupabase({
    url: env.SUPABASE_URL,
    anonKey: env.SUPABASE_ANON_KEY,
    serviceRoleKey: env.SUPABASE_SERVICE_ROLE_KEY
 ...
 */
function extractBusinessIdFromApiKey(apiKey: string): string | null {
