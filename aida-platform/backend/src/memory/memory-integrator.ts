...
export interface MemoryIntegratorConfig {
  ...
  maxConversationContext: number;
  maxKnowledgeContext: number;
...
export class MemoryIntegrator {
  ...
  constructor(
    ...
    this.conversationManager = new ConversationHistoryManager(
      supabase, 
      businessId, 
      config.embeddingConfig
    );

    this.knowledgeManager = new BusinessKnowledgeManager(
      supabase, 
      businessId, 
      config.embeddingConfig
    );
  ...
  async getMemoryContext(
    ...
    try {
      ...
      const knowledgeResults = await this.knowledgeManager.searchKnowledge(
        ...
        this.config.maxKnowledgeContext,
        0.7 // Similarity threshold
      );

      // Search conversation history for similar past interactions
      const similarConversations = await this.conversationManager.searchConversationHistory(
        currentMessage,
        assistantId,
        5
      );
  ...
  async queryMemory(query: MemoryQuery): Promise<MemoryResult> {
    try {
      const {
        ...
        conversationId,
        assistantId,
        focusAreas = ['conversation', 'knowledge'],
        maxResults = 10,
        timeWindow
      } = query;
      ...
      if (focusAreas.includes('conversation')) {
        const conversationResults = await this.conversationManager.searchConversationHistory(
          text,
          assistantId,
      ...
      results.integratedResults = this.combineAndRankResults(
  ...
  async updateBusinessKnowledge(
    ...
    try {
      return await this.knowledgeManager.createKnowledgeNode(
        entityType,
        entityName,
        properties,
        content,
  ...
  async optimizeMemory(): Promise<{
    ...
    knowledgeNodesOptimized: number;
  ...
  private buildContextSummary(
    ...
    knowledgeResults: Array<{ node: KnowledgeNode; similarity: number }>,
    similarConversations: Array<any>
  ): string {
    const parts: string[] = [];
   ...
   */
  private extractEntities(text: string): Array<{ type: string; name: string; confidence: number }> {
...
export function createMemoryIntegrator(
  supabase: TenantAwareSupabase,
  businessId: string,
  config: Partial<MemoryIntegratorConfig> = {}
): MemoryIntegrator {
  const defaultConfig: MemoryIntegratorConfig = {
 ...
 */
export function getDefaultMemoryConfig(): MemoryIntegratorConfig {
  return {
    conversationWeight: 0.6,
    knowledgeWeight: 0.4,
    maxConversationContext: 10,
