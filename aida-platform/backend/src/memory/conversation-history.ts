...
export class ConversationHistoryManager {
  ...
  constructor(supabase: TenantAwareSupabase, businessId: string, embeddingConfig: any) {
  ...
  async addMessage(
    ...
    messageType: 'text' | 'media' | 'location' = 'text',
  ...
  async searchConversationHistory(
    ...
    assistantId?: string,
    limit: number = 5
  ): Promise<Array<{ conversation: Conversation; message: Message; similarity: number }>> {
    try {
  ...
  async updateConversationSummary(conversationId: string): Promise<void> {
    try {
      ...
      const summaryRecord: Partial<ConversationSummary> = {
        conversation_id: conversationId,
   ...
   */
  async buildCustomerProfile(remoteJid: string): Promise<CustomerProfile> {
    try {
      ...
      const customerMessages = conversations?.flatMap(conv => 
        conv.messages?.filter((m: any) => m.sender_type === 'customer') || []
      ) || [];
   ...
   */
  private async updateConversationAfterMessage(conversationId: string, content: string): Promise<void> {
    try {
   ...
   */
  private async generateConversationSummary(conversationText: string): Promise<{
    content: string;
    keyTopics: string[];
   ...
   */
  private detectLanguage(messages: any[]): string {
    ...
    const ptCount = portugueseWords.reduce((count, word) => 
      count + (text.includes(word) ? 1 : 0), 0);
    const enCount = englishWords.reduce((count, word) => 
      count + (text.includes(word) ? 1 : 0), 0);
   ...
   */
  private analyzeConversationStyle(messages: any[]): 'formal' | 'casual' | 'standard' {
    // Simple style analysis
    const text = messages.map(m => m.content).join(' ').toLowerCase();
    
    if (text.includes('senhor') || text.includes('vossa')) {return 'formal';}
    if (text.includes('😊') || text.includes('rs') || text.includes('kk')) {return 'casual';}
    return 'standard';
  }
   ...
   */
  private extractPreviousIssues(conversations: any[]): string[] {
 ...
 */
export function createConversationHistoryManager(
