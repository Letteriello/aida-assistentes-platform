...
export class BusinessKnowledgeManager {
  ...
  async createKnowledgeNode(
    ...
    try {
      ...
      const knowledgeNode: Partial<KnowledgeNode> = {
        id: nodeId,
  ...
  async createKnowledgeRelationship(
    ...
    try {
      ...
      const relationship: Partial<KnowledgeRelationship> = {
        id: relationshipId,
        business_id: this.businessId,
   ...
   */
  async queryKnowledgeGraph(query: GraphQuery): Promise<GraphResult> {
    try {
      const {
        startNodeId,
        relationshipTypes = [],
  ...
  async ingestKnowledge(
    ...
    try {
      ...
      const createdNodes: Map<string, KnowledgeNode> = new Map();
      ...
      for (const item of data) {
        try {
          ...
          const existingNode = await this.findKnowledgeNodeByName(
            item.entityType, 
            item.entityName
          );
          ...
          if (existingNode) {
            ...
            // Create new node
            node = await this.createKnowledgeNode(
              item.entityType,
              item.entityName,
              item.properties,
              item.content,
  ...
  async updateKnowledgeNode(
    nodeId: string,
    updates: Partial<KnowledgeUpdate>
  ): Promise<KnowledgeNode> {
    try {
      // Get current node
   ...
   */
  private async storeNodeVersion(node: KnowledgeNode): Promise<void> {
    try {
      const versionRecord = {
        node_id: node.id,
        business_id: this.businessId,
 ...
 */
export function createBusinessKnowledgeManager(
